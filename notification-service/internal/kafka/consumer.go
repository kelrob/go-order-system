...
func (c *Consumer) Start(ctx context.Context) error {
	...
	for {
		...
		fetches.EachRecord(func(record *kgo.Record) {
			...
			c.appLog.Error(envelope.TraceId, record.Topic, "Message failed, sending to DLQ", nil, map[string]any{
				"max_retries": c.maxRetry,
			})
			dlqErr := c.dlq.Send(ctx, record.Topic, record.Value)
			if dlqErr != nil {
				c.appLog.Error(envelope.TraceId, record.Topic, "Failed to send to DLQ", dlqErr, nil)
			}
		})
	}
}

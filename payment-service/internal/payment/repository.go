...
func NewRepository(db *pgxpool.Pool, appLog *logger.Logger) *Repository {
	return &Repository{db: db, breaker: circuit.NewBreaker(3, 10*time.Second, appLog), appLog: appLog}
}

func (s *Repository) ProcessPayment(ctx context.Context, event InventoryReservedEvent) error {
	return s.breaker.Execute(func() error {
		...
		if paymentSucceeded {
			...
			outboxPayload, err = json.Marshal(PaymentFailedEvent{
				TraceId:   event.TraceId,
				OrderId:   event.OrderId,
				UserId:    event.UserId,
				CreatedAt: time.Now(),

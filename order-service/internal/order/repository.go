...
func (s *Repository) GetOrder(ctx context.Context, id string) (Order, error) {
	var order Order
...
func (s *Repository) GetOrders(ctx context.Context) ([]Order, error) {
	rows, err := s.db.Query(ctx,
		`SELECT
...
func (s *Repository) UpdateOrderStatus(ctx context.Context, orderId string, status OrderStatus) error {
	_, err := s.db.Exec(ctx,

...
import (
	...
	"github.com/kelrob/inventory-service/config"
	"github.com/kelrob/inventory-service/domain"
	"github.com/kelrob/inventory-service/kafka"
	"github.com/kelrob/inventory-service/storage"
...
var (
	db             *pgxpool.Pool
	inventoryStore *storage.InventoryStore
...
func handleOrderCreated(data []byte) error {
	...
	err = inventoryStore.ReserveInventory(context.Background(), event)
	if err != nil {
...
func handlePaymentFailed(data []byte) error {
	...
	err = inventoryStore.UnreserveInventory(context.Background(), event.OrderId, event.TraceId, event.Items)
	if err != nil {
...
func main() {
	...
	inventoryStore = storage.NewInventoryStore(db)
	...
	go func() {
		appLog.Log("Listening on port 8081", nil)
